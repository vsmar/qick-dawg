...
class RFTest_CPMG(NVAveragerProgram):
    ...
    def initialize(self):
        ...
        # CPMG waveform
        for i in range(self.cfg.waveforms):
            idata = np.zeros(2**16)
            qdata = np.zeros(2**16)
            ...
            self.add_envelope(ch=self.cfg.mw_channel, name=f"measure_{i}", idata=idata, qdata=qdata)
        self.set_pulse_registers(ch=self.cfg.mw_channel, style="arb", waveform="measure_0",
    ...
    def body(self):
        ...
        for i in range(self.cfg.waveforms):
            self.set_pulse_registers(ch=self.cfg.mw_channel, style="arb", waveform=f"measure_{i}",
                                     freq=self.cfg.mw_freg, phase=0, gain=self.cfg.mw_gain)
