...
class CPMGXYFineRes(ReadoutHelpers, NVAveragerProgram):
    ...
    def initialize(self):
        ...
        self.tau_samples = self.new_gen_reg(self.cfg.mw_channel, "tau_samples", init_val=self.cfg.tau_start_ftsamp)
        self.add_sweep(NVQickSweep(self, self.tau_samples, self.cfg.tau_start_ftsamp, self.cfg.tau_end_ftsamp, self.cfg.nsweep_points))
    ...
    def offset_computations(self, double_tau=False):
        ...
        if double_tau: # Add another tau of delay (= 2*tau)
            self.math(self.ftsamp_offset_register.page, self.ftsamp_offset_register.addr, 
                      self.ftsamp_offset_register.addr, "+", self.tau_samples.addr)
        # account for dead time in pi pulse waveform (subtract the unused portion of the pi pulse)
