...
class PODMR2(NVAveragerProgram):
    '''
    TODO:
    ...
    def analyze_results(self, data):
        ...
        returns
            ...
            .background (nfrequency np.array, adc units)
                - average adc signal without MW pulse (reference)
            .contrast (nfrequency np.array, fractional units)
                - (signal - background) / background
            .mid_rep_reinit_counts (nfrequency np.array, adc units)
                - reinitialization counts after first readout
            .end_rep_reinit_counts (nfrequency np.array, adc units)
                - reinitialization counts at end of rep
        """
        data = np.reshape(data, self.data_shape)
        
        d = ItemAttribute()
        d.signal = data[..., 0]
        d.reinit_counts = data[..., 1]
        d.reference = data[..., 2]
        d.reinit_counts2 = data[..., 3]
        
        # Calculate contrast: (signal - background) / background
        d.contrast = (d.signal - d.reference) / d.reference
        
        # Average over all axes except the last (frequency) axis
        n = len(d.signal.shape) - 1
        
        for key in ['signal', 'reference', 'contrast', 
                    'reinit_counts', 'reinit_counts2']:
...
# MW pulse
config.mw_nqz = 1 # 1 < 2.4925 GHz < 2 < 4.985 GHz
config.mw_pi_tns = 130
config.mw_gain = 12000
config.add_linear_sweep('mw', 'fMHz', start=2680, stop=2740, delta=1)

# Reps and Delays
config.pre_init = True 
config.reps = 50000

config.laser_on_tus = 4
config.laser_readout_offset_tus = 1.14
config.readout_integration_tns = 500
config.readout_reference_start_tus = 2
config.mw_laser_delay_treg = 0

config.relax_delay_tus = 1.1


prog = qd.PODMR(config)
d = prog.acquire()
...
# Plot contrast
true_contrast = d.signal / d.reference
ax1.plot(d.frequencies, true_contrast, label="contrast", color="tab:green")
ax1.set_ylabel("contrast")

# Plot signal and reference
ax2 = ax1.twinx()
ax2.plot(d.frequencies, d.signal, label="signal", color="tab:blue",
         alpha=0.6, linestyle=":")
ax2.plot(d.frequencies, d.reference, label="reference", color="tab:orange",
         alpha=0.6, linestyle=":")
ax2.plot(d.frequencies, d.mid_rep_reinit_counts, label="reference", color="tab:red",
         alpha=0.6, linestyle=":")
...
# --- Dip finding ---
def find_dips(signal):
    inv_signal = -signal
    peaks = np.where((inv_signal[:-2] < inv_signal[1:-1]) &
                     (inv_signal[1:-1] > inv_signal[2:]))[0] + 1
    sorted_peaks = sorted(peaks, key=lambda p: inv_signal[p], reverse=True)
    return sorted_peaks[:num_dips]

dips = find_dips(true_contrast)

# Plot dips on contrast axis
ax1.plot(d.frequencies[dips], true_contrast[dips], 'ro', label='dips')

# Annotate dips
for dip in dips:
    ax1.annotate(
        f"{d.frequencies[dip]:.2f} MHz",
        xy=(d.frequencies[dip], true_contrast[dip]),
        xytext=(0, -11),
...
# Title + x‑label on the main axis
ax1.set_title(r"P-ODMR Spectrum: $\mathit{D_o}$")
