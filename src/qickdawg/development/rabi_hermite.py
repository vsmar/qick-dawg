...
from .hermite_envelope import HermiteEnvelope
import numpy as np
...
# 2**16 samples of waveform memory, 2**12 treg worth of waveform memory
# We can only section memory in treg, and the shortest pulse is 3
WVFM_MEMORY_TREG = 2**12
WVFM_MEMORY_SAMPS = 2**16

# for simplicity we will force the first waveform to be 3 or larger
if start_ciel_treg < 3:
    ValueError()"`start_ciel_treg` Must be 3 treg (48 samples) or larger")

# We will increment by treg multiple each time
def compute_max_num_waveforms(start, stride, end=None):
    N = ( (stride - 2*start) + np.sqrt((2*start-stride)**2 + 4 * stride * 2**13) ) / (2 * stride)
    if N < 1:
        ValueError(f"start value is too large")
    if end is not None:
        max_end = N*stride + start
        if max_end < end:
            ValueError(f"Maximum possible end value is {max_end} treg")

for length in np.arange(start, stop, step):
    HermiteEnvelope(duration, amplitude, type)
