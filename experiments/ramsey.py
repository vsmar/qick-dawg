...
config.add_exponential_sweep(
    ...
    stop=TAU_STOP_FTNS,
    scaling_factor=TAU_SCALING_FACTOR,
)

print(
    f"[ramsey] Sweep: {config.tau_start_ftns:.3f} -> {config.tau_end_ftns:.3f} ftns "
    f"(exponential, scaling_factor={TAU_SCALING_FACTOR}, configured delta={config.tau_delta_ftns:.3f})"
)
print(
...
if not hasattr(data, "tau_ftns"):
    raise ValueError("Ramsey output missing expected sweep axis tau_ftns.")
...
if PLOT_DEBUG_RAW:
    plot_debug_traces(
        ...
        title=f"Ramsey Debug Raw | {timestamp}",
        metadata=metadata,
