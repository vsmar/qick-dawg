...
if PLOT_DEBUG_RAW:
    plot_debug_traces(
        ...
        title=f"T1 Debug Raw | pi - delay - readout | {timestamp}",
        metadata=metadata,
...
ax_top.set_title(f"T1 | MW on/off and difference | {timestamp}")
...
metadata_line = (
    f"run_id={run_id} | gain={config.mw_gain} | pi_ftsamp={config.mw_pi_ftsamp} | "
    f"pi_ftns={config.mw_pi_ftns:.2f} | reps={config.reps} | "
    f"laser_mW={cfg['optics']['excitation_laser_power_mW']} | "
    f"units={'cts/s' if PLOT_USE_COUNTS_S else 'raw'}"
)
